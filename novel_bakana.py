...
class BakaNa(AbstractNovel):
    ...
    @staticmethod
    def check_url(url):
        url_checker = re.compile(r'http://www.8kana.com/book/(\d+).html')
    ...
    def get_chapter_content(src):
        i = src.select('div.myContent')[0].find_all('p')
        lines = []
        for j in i[1:]:
            v=j.text.strip()
            if not 'www.8kana.com' in v:
                lines.append(v)
        return lines
    ...
    def extract_chapter(self, url, number):
        ...
        try:
            ...
            chapter_name = soup.select('h2.readbook_title')[0].text
    ...
    def extract_volume_name(self, src):
        title= src.contents[1].text.strip()
    ...
    def extract_volume_url(self, src):
        links_tag=src.find('div',{'class':'mainList_In'}).contents[1].find_all('a')
        links=[]        
        for index in range(len(links_tag)):            
            links.append('http://www.8kana.com'+links_tag[index].attrs['href'])        
    ...
    def extract_common_information(self, soup):
        self.book_name=soup.select('div.bookContain_Title')[0].text.strip()
        self.author = soup.select('a.authorName')[0].text
        self.introduction = soup.select('div#li_booknote')[0].text.strip()
        self.cover_url = soup.select('div.bookContain_Cover')[0].find('img').attrs['src']
        self.find_date(soup)

    def parse_vollist(self):
        soup = self.parse_page(self.url)
        volumes=soup.find_all('div',{'class':'mainList','flag':'volumes'})
